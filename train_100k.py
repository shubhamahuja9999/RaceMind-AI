"""Phase 4: 100K timestep PPO training run.

Scaled up from the 10K smoke test. Evaluates every 20K steps and saves
checkpoints. Expect this to take ~30-40 minutes on CPU.
"""
...
def main() -> None:
    ...
    ppo_config = PPOConfig()  # CnnPolicy, lr=3e-4, n_steps=2048
    ...
    checkpoint_dir = sim_config.checkpoints_dir / "ppo_100k"
    tb_log_dir = Path("runs") / "ppo_100k"

    _logger.info("=" * 60)
    _logger.info("Phase 4 — PPO 100K Training Run")
    _logger.info("=" * 60)
    ...
    trainer = build_ppo_trainer(
        ...
        seed=42,
        experiment_name="ppo_100k",
    )

    summary = trainer.train()

    _logger.info("=" * 60)
    _logger.info("100K Training Results")
    _logger.info("=" * 60)
