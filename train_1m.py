"""Phase 4: 1M timestep PPO training run.

Final scale-up. Evaluates every 100K steps and saves checkpoints.
Expect this to take ~7-8 hours on CPU — ideal as an overnight run.
"""
...
def main() -> None:
    ...
    checkpoint_dir = sim_config.checkpoints_dir / "ppo_1m"
    tb_log_dir = Path("runs") / "ppo_1m"

    _logger.info("=" * 60)
    _logger.info("Phase 4 — PPO 1M Training Run")
    _logger.info("=" * 60)
    ...
    trainer = build_ppo_trainer(
        ...
        seed=42,
        experiment_name="ppo_1m",
    )

    summary = trainer.train()

    _logger.info("=" * 60)
    _logger.info("1M Training Results")
    _logger.info("=" * 60)
    _logger.info("Total timesteps:   %d", summary.total_timesteps)
    _logger.info("Elapsed:           %.1fs (%.1f min / %.1f hrs)",
                 summary.elapsed_seconds,
                 summary.elapsed_seconds / 60,
                 summary.elapsed_seconds / 3600)
    ...
    for step, reward in summary.evaluations:
        _logger.info("  step=%7d  reward=%.2f", step, reward)
